    ...
    >
     ...
     <tbody>
      ...
      >
       ...
       <td>
        {{ certification_request.certification.certification_description }}
       </td>
       ...
       <td v-if="permission == 'resident' && certification_request.status == 'Pending'">
        <button
         class="btn btn-primary"
       ...
       <td v-if="(permission == 'chairperson' && certification_request.status == 'Pending') || (permission == 'secretary' && certification_request.status == 'Pending')">
        <button
       ...
       <td v-if="certification_request.status == 'Approved'">
        ...
        <button
         class="btn btn-success"
         @click="findGcashToGenerate(certification_request)"
         v-if="new Date(moment(certification_request.date).format('LL')).getTime() <= new Date(moment(today).format('LL')) && (permission == 'chairperson' || permission == 'secretary')"
        >
   ...
   >
    ...
    <div class="form-group">
     ...
     <div v-if="errors.expected_date">
      <label style="color: red; font-weight: 500">{{ this.errors.expected_date }}</label>
     </div>
    </div>
    <div class="form-group">
     <label
     ...
     <div v-if="errors.expected_time">
      <label style="color: red; font-weight: 500">{{ this.errors.expected_time }}</label>
     </div>
    </div>
   </b-modal>
...
export default {
 ...
 methods: {
  async fetchFindResidentCertificationRequest() {
   this.loading = true;
   await axios
    .post(`/find-resident-certificate?page=${this.currentPage}`, this.resident_id)
    .then((response) => {
  ...
  async createCertificationRequest() {
   if (new Date() > new Date(moment(this.expected_date).format("L")) && !this.isToday(new Date(this.expected_date))) {
    this.errors = {
  ...
  async findGcashToGenerate(data) {
   ...
   await axios.post(`/search-gcash`, datas).then((response) => {
    if (response.data.length != 0) {
     if (response.data[0].certification_request.certification_id == 1) {
      this.$router.push({
       path: `/certificate/barangay_certificates/${response.data[0].certification_request.id}`,
      });
     } else if (response.data[0].certification_request.certification_id == 2) {
      this.$router.push({
       path: `/certificate/barangay_clearance/${response.data[0].certification_request.id}`,
      });
     } else if (response.data[0].certification_request.certification_id == 4) {
      this.$router.push({
  ...
  async approveRequest() {
   ...
   if (new Date().getTime() >= new Date(moment(this.date).format("L")).getTime() && !this.isToday(new Date(this.date))) {
    ...
    await axios.put(`/certification_request/${data.id}`, data).then((response) => {
  ...
  async approvedCertificates() {
   const data = {
    ...
    date: new Date(new Date(this.payment_date).getTime() - new Date().getTimezoneOffset() * 60000).toISOString().split("T")[0],
   };

   await axios.post(`/certification-request`, data).then((response) => {
    if (this.permission == "chairperson" || this.permission == "secretary") {
     this.$toast.success("Certification Request has been updated.");
     this.fetchCertificationRequest();
     this.resetFields();
