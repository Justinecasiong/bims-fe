    ...
    <div class="certification_request">
      ...
      <b-modal id="modal-payment-view" size="md" title="Payment Review" centered @ok.prevent="approvedCertificates()"
        ...
        <div class="text-center">
          <img
            :src="'http://13.229.52.4:8080/evidence/' + this.evidence"
            width="300"
          />
        </div>
      </b-modal>
...
export default {
  ...
  methods: {
    ...
    async fetchCertificationRequest() {
      ...
      await axios
        .get(
          `/certification_request?page=${this.currentPage}&status=${this.search_status}`
        )
        .then((response) => {
          this.certification_requests = response.data.data;
      ...
      await axios.post(`/search-gcash`, datas).then((response) => {
        if (response.data.length != 0) {
          if (response.data[0].certification_request.certification_id == 1) {
            ...
            response.data[0].certification_request.certification_id == 2
          ) {
            this.$router.push({
              path: `/certificate/barangay_clearance/${response.data[0].certification_request.id}`,
            });
          } else if (
            response.data[0].certification_request.certification_id == 4
          ) {
            this.$router.push({
