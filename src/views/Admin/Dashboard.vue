<template>
  <div class="container">
    ...
    <div class="container">
          ...
          <div class="row mb-4" v-if="certification.status == 'Unpaid Certificates'">
            <h4 style="font-weight: lighter">
              <span>
                Your request for
                {{ certification.certification.certification_description }}
                has been processed. Please choose a mode of payment.
              </span>
            </h4>
          </div>
          <div class="row mb-4" v-if="certification.status == 'Paid'">
            <h4 style="font-weight: lighter">
              <span>
                Your payment for
                {{ certification.certification.certification_description }}
                is being validated. Please wait for the confirmation for your
                request.
              </span>
            </h4>
          </div>
      ...
      <div class="row card">
        <h6>
          > Barangay 6-A is one of the Barangays of Tacloban City. > Tacloban
...
export default {
    ...
    return {
      ...
      chartEvents: {
        select: () => {
          ...
          if (selection[0].column == 1 && this.permission != "resident") {
            this.$router.push({
  ...
  methods: {
    async fetchFindResidentCertificationRequest() {
      ...
      await axios
        .post(`/find-resident-certificate?page=${this.currentPage}`, data)
        .then((response) => {
    ...
    async fetchEducationCount() {
      this.loading = true;
      await axios.get(`/educational-attainment`).then((response) => {
        let data = response.data;
        this.edAttainments.push(
          ['Year', 'Day Care', 'Nursery', 'Kinder', 'Elementary Level', 'Elementary Grad', 'ALS High School', 'High School Level', 'High School Grad', 'Senior High', 'Vocational', 'College Level', 'College Grad', 'Post Graduate'],
          ["2023", data.day_care, data.nursery, data.kinder, data.elementary_level, data.elementary_grad, data.als_high_school, data.high_school_level, data.high_school_grad, data.senior_high, data.vocational, data.college_level, data.college_grad, data.post_graduate]
        )
