  ...
  <b-modal id="modal-gcash" size="md" title="GCASH Payment" centered @ok.prevent="gcash.length != 0 ? updateGcash() : createGcash()" @hidden="resetFields()" ok-title="Save">
   <div class="row">
    ...
    <div class="text-center mb-3" v-if="gcash.length != 0 && image == null">
     <img :src="'http://13.229.52.4:8080/qr_code/' + this.gcash[0].image" width="300" />
    </div>

    <div class="form-group text-center">
     <label class="control-label">Attach GCASH QR Code:</label>
     <input class="form-control" v-on:change="onProfilePicChange($event)" type="file" id="imageFile" accept="image/*" />
    </div>
   </div>
  </b-modal>
  <b-modal id="view-gcash" size="md" title="GCASH Payment" centered ok-title="Close" ok-only>
   <div class="row">
    <div class="text-center mb-3" v-if="image != null">
     <img :src="image" width="300" />
    </div>
