<template>
 <div class="container">
    ...
    <div class="row mt-3">
     ...
     <div class="col-2" v-if="permission == 'chairperson' || permission == 'kagawad' || permission == 'secretary' || permission == 'treasurer'">
      <div style="margin: auto; position: absolute; top: 35px; right:50px" v-if="permission != 'resident'">
       ...
       <button class="btn btn-danger" v-b-modal.modal-delete @click="setPosition(announcement)">
  ...
  <b-modal class="add_modal" id="modal-add" title="Create Announcement" size="lg" centered @ok.prevent="createAnnouncement()" @cancel.prevent="resetFields()" @hidden="resetFields()" ok-title="Save">
   <div class="row mt-3">
    <div class="form-group">
     <label for="complainant" class="control-label"> What: </label>
     <textarea v-model="what" type="text" class="form-control" id="complainant"></textarea>
    </div>
   </div>
   <div class="row mt-3">
    <div class="form-group">
     <label for="complainant" class="control-label"> Who: </label>
     <textarea v-model="who" type="text" class="form-control" id="complainant"></textarea>
    </div>
   </div>
   <div class="row mt-3">
    <div class="form-group">
     <label for="complainant" class="control-label"> When: </label>
     <textarea v-model="when" type="text" class="form-control" id="complainant"></textarea>
    </div>
   </div>
   <div class="row mt-3">
    <div class="form-group">
     <label for="complainant" class="control-label"> Where: </label>
     <textarea v-model="where" type="text" class="form-control" id="complainant"></textarea>
    </div>
   </div>
   <div class="row mt-3">
    <div class="form-group">
     <label for="complainant" class="control-label"> Why: </label>
     <textarea v-model="why" type="text" class="form-control" id="complainant"></textarea>
    </div>
   </div>
   <div class="row mt-3">
    <div class="form-group">
  ...
  <b-modal class="edit_modal" id="modal-edit" title="Edit Announcement" size="lg" centered @ok.prevent="updateAnnouncement()" ok-title="Update">
   <div class="row mt-3">
    <div class="form-group">
     <label for="complainant" class="control-label"> What: </label>
     <textarea v-model="what" type="text" class="form-control" id="complainant"></textarea>
  ...
  <b-modal id="modal-delete" size="md" title="Delete Announcement" centered @hidden="resetFields()" @ok.prevent="deleteAnnouncement()" ok-title="Delete"> Are you sure you want to delete this data? </b-modal>
 </div>
</template>
 ...
 export default {
  ...
  methods: {
   ...
   async deleteAnnouncement() {
    await axios.delete(`/announcement/${this.id}?remember_token=${localStorage.getItem("token")}`).then(() => {
