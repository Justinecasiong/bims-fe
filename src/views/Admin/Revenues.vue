    ...
    <div class="page-header">
      ...
      <div class="row">
        ...
        <div class="col mt-5" v-if="permission == 'treasurer'">
          <div
            style="text-align: right; margin-bottom: 20px; margin-right: 50px"
          >
            <vue-excel-xlsx
      ...
      >
        ...
        <tbody>
          <tr v-for="revenue in revenues" :key="revenue.id">
            <th scope="row">
              {{ moment(revenue.created_at).format("LL") }}
            </th>
            <td>
              {{
                revenue.residents
    ...
    async exportExcel(data) {
      ...
      await axios
        ...
        .then((response) => {
          ...
          response.data.forEach((revenue) => {
            this.json_data.push({
              date: moment(revenue.created_at).format("LL"),
              recipient: revenue.residents
                ? revenue.residents.first_name +
                  " " +
                  revenue.residents.last_name
                : revenue.custom_recipient,
              payment_details: revenue.details,
              mode_of_payment: revenue.mode_of_payment,
